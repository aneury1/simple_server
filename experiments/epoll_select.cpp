    ...
    while (true) {
        ...
        if (activity > 0) {
            ...
            if (FD_ISSET(backendSocket, &readfds)) {
                ...
                if (bytesRead <= 0) break; // Backend disconnected
...
int main() {
    ...
    struct sockaddr_in masterAddr{}, clientAddr{};
    ...
    while (true) {
        ...
        for (int i = 0; i < ready; ++i) {
            if (events[i].data.fd == masterSocket) {
                // Accept new client
                int clientSocket = accept(masterSocket, (struct sockaddr*)&clientAddr, &addrLen);
                ...
                if (bytesPeeked > 0) {
                    ...
                    std::string protocol = "PROTOCOL1"; // Dummy logic: Parse actual protocol here
