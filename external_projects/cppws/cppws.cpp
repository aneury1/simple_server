...
bool sendWebSocketFrame(int socket_fd, const std::vector<uint8_t>& payload, uint8_t opcode) {
...
bool receiveWebSocketFrame(int socket_fd) {
    ...
    while (total_received < payload_length) {
        ssize_t received = recv(socket_fd, payload.data() + total_received, payload_length - total_received, 0);
        if (received <= 0) {
...
std::pair<std::string,std::string> split_by(std::string line, std::string where){
...
std::map<std::string, std::string> extract_request(int socke){
   std::map<std::string, std::string> ret;
    ...
    do{
	  ...
	  std::cout <<"Header: "<<splitted.first<<"->"<<" Value: "<<splitted.second<<"\n";
       }  
     }while(!line.empty());
    return ret;
}
...
bool do_ws_handshake(int client, std::map<std::string, std::string> headers){
    ...
    std::cout << response <<"\n\n\n";
    std::cout << "Handshake successful. Response sent (" << sent_bytes << " bytes)." << std::endl;
    return true;
}
...
std::string read_frame(int client_socket){
  ...
  if ((d = recv(client_socket, header, 2, 0)) != 2) {
        std::cerr << "Failed to read frame header."<<d <<" " <<client_socket << std::endl;
        return "";
    }
    ...
    if (payload_length == 126) {
        ...
        if (recv(client_socket, extended_payload, 8, 0) != 8) {
            std::cerr << "Failed to read extended payload length (64-bit)." << std::endl;
            return "";
        }
        payload_length = 0;
        for (int i = 0; i < 8; ++i) {
    ...
    while (bytes_read < payload_length) {
        ssize_t result = recv(client_socket, payload.data() + bytes_read, payload_length - bytes_read, 0);
        if (result <= 0) {
